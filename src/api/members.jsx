...
const getMembers = async (token) => {
  try {
    const response = await fetch(`${process.env.REACT_APP_API_URL}/api/v1/members`, {
      method: 'GET',
      headers: {
...
const getMemberById = async (token, id) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/v1/members/${id}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
...
const updateMember = async (token, memberId, newMember) => {
   const response = await fetch(`${process.env.REACT_APP_API_URL}/api/v1/members/${memberId}`, {
     method: 'PUT',
     headers: {
       'Authorization': `Bearer ${token}`
     },
