...
const getPaymentsAsso = async (token) => {
    ...
    try {
        const reponse = await fetch ('http://api.g-asso.com/api/v1/paiement/views', {
            method: 'GET',
            headers: {
...
const createNewTransac = async (token, formValue) => {
    ...
    try{
        const reponse = await fetch('http://api.g-asso.com/api/v1/paiement', {
            method: 'POST',
            headers: {
                'Content-type': 'application/json',
                'Authorization': 'Bearer ' + token,
...
const getBalanceAsso = async (token) => {
    ...
    try {
        const response = await fetch('http://api.g-asso.com/api/v1/paiement/view/balance', {
            method : 'GET',
            headers: {
                'Content-type': 'application/json',
                'Authorization': 'Bearer ' + token
