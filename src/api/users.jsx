...
const getUserById = async (token, id) => {
    try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/v1/users/${id}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
const getGroupsByUserId = async (token, id) => {
    try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/v1/users/${id}/groups`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
